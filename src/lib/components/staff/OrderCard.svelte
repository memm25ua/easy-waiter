...
<article class="ew-panel p-4">
  <div class="flex items-start justify-between gap-3">
    <div>
      ...
      <p class="ew-muted text-sm">{order.source === 'ai' ? d['orders.aiAssisted'] : d['orders.manual']}</p>
    </div>
  ...
  {#if order.customerNotes}<p class="ew-muted mt-2 text-sm">{order.customerNotes}</p>{/if}
